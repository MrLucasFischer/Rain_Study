...
We've checked some of the features and found out visually that `RainToday`, `RainFall` and possibily `Humidity3pm` are good features to use in our machine learning model but other features might be as well. To determine which features are relevant for our model we'll use *Recursive Feature Elimination* from the `caret` package

```{r}
train_idx = createDataPartition(y = data$RainTomorrow, p = 0.8, list = F)

train_data = data[train_idx, ]
test_data = data[-train_idx, ]

filter_ctrl = sbfControl(
    functions = rfSBF,
    method = "cv",
    number = 5,
    verbose = T
